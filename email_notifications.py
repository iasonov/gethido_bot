...
def build_text_body(
    ...
    lines = [
        "Добрый день!",
        "",
        f"По итогам студенческой оценки преподавания за {module_label} по программе "
        f"«{program_summary['program']}» зафиксирован статус: {program_summary['status']}.",
        "",
        "Дисциплины, на которые стоит обратить внимание:",
    ]
...
<html>
  <body>
    <p>Добрый день!</p>
    <p>
      По итогам студенческой оценки преподавания за {html.escape(module_label)} по программе
      «{html.escape(program_summary['program'])}» зафиксирован статус:
      <strong>{html.escape(program_summary['status'])}</strong>.
    </p>
